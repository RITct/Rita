...
def action_train(n_iters, training_data):
    ...
    with open('app/brain/action_meta.pkl','rb') as pickle_file:
    ...
    torch.save(ann, 'app/brain/ann.pt')

    metadata = open('app/brain/action_meta.pkl', 'wb')
...
def action_predict(sentence):
    ann = torch.load('app/brain/ann.pt')
    with open('app/brain/action_meta.pkl','rb') as pickle_file:
         meta = pk.load(pickle_file)
